...
class AngleDegModel(nn.Module):
    ...
    def predict_angles(self, loader: DataLoader, device: torch.device) -> np.ndarray:
        ...
        print(len(np.concatenate(preds)), len(np.concatenate(trues)), len(np.concatenate(all_images)))
...
@dataclass
class TrainConfig:
    out_dir: str = "rotate_finetune_model_test"
    image_size: int = 640
    batch_size: int = 32
    epochs: int = 200
    lr: float = 3e-4
    ...
    num_workers: int = 4
    amp: bool = True
    resume: str = "rotate_finetune_100e/best_model.pth"

...
import cv2
import numpy as np
from rotation_model.rotate_network import AngleDegModel, TrainConfig, load_checkpoint
...
def train():
    ...
    train_ds = PageAngleDataset(
        ...
        image_bboxes=get_bbox_vectors("datasets/yolo-split_per_book/labels/train"),
        is_train=True,
        image_size=cfg.image_size,
        angle_max=cfg.angle_max,
    )
    val_ds = PageAngleDataset(
        image_paths=get_filepaths("datasets/yolo-split_per_book/images/val"),
        image_bboxes=get_bbox_vectors("datasets/yolo-split_per_book/labels/val"),
        is_train=True,
        image_size=cfg.image_size,
        angle_max=cfg.angle_max,
    )
...
def test():
    ...
    test_ds = PageAngleDataset(
        image_paths=get_filepaths("datasets/yolo-split_per_book/images/test"),
        image_bboxes=get_bbox_vectors("datasets/yolo-split_per_book/labels/test"),
        is_train=True,
        image_size=640,
        angle_max=10.0,
    )
    ...
    for img, pred, true in zip(imgs, preds, trues):
        ...
        img_np = np.clip((img_np * 0.25) + 0.5, 0, 1)  # inverse normalize: std=0.25, mean=0.5

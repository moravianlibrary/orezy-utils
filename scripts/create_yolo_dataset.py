    ...
    for json_file in tqdm.tqdm(files):
        ...
        with open(path, 'r') as f:
            ...
            for k, v in data.items():
                max_rotation_in_dataset = max(max_rotation_in_dataset, max(abs(crop['rotation']) for crop in v['crop']))
                if v["split"] is None:
                    continue
                if abs(v["crop"][0]["rotation"]) < min_rotation or abs(v["crop"][1]["rotation"]) < min_rotation:
                    continue
                if abs(v["crop"][0]["rotation"]) > max_rotation or abs(v["crop"][1]["rotation"]) > max_rotation:
                    continue
                ...
                v["crop"][1]["x"] = v["crop"][1]["x"] + min(v["split"]["p1"][0], v["split"]["p2"][0])
                h, w = img.shape[:2]
                for crop in v["crop"]:
                    x1 = crop["x"]
                    y1 = crop["y"]
                ...
                st.image(img, caption=f"Image with bboxes, rotation={max(abs(crop['rotation']) for crop in v['crop'])}", width=400)
...
def process_project_double_page(input, output, project_name, metadata_folder, split="train"):
    ...
    for image_name in metadata:
        ...
        if img_properties["split"] is None:
            continue
        if abs(img_properties["crop"][0]["rotation"]) > 0.25 or abs(img_properties["crop"][1]["rotation"]) > 0.25:
            continue
        ...
        page_name = f"double_{project_name}-{image_name.replace('/', '_').split('.')[0]}"
        ...
        with open(os.path.join(output, "labels", split, f"{page_name}.txt"), 'w') as f:
            ...
            img_properties["crop"][1]["x"] = img_properties["crop"][1]["x"] + min(img_properties["split"]["p1"][0], img_properties["split"]["p2"][0])
...
def process_project_single_page(input, output, project_name, metadata_folder, split="train"):
    ...
    for image_name in metadata:
        ...
        if img_properties["split"]:
            left_divider = max(img_properties["split"]["p1"][0], img_properties["split"]["p2"][0])
            right_divider = min(img_properties["split"]["p1"][0], img_properties["split"]["p2"][0])
        ...
        for i, page in enumerate(pages):
            page_name = f"{project_name}-{image_name.replace('/', '_').split('.')[0]}_p{i}"
            # save bbox info in YOLO format
            with open(os.path.join(output, "labels", split, f"{page_name}.txt"), 'w') as f:
                normalized_bbox = convert_bbox_to_yolo_format(img_properties["crop"][i], page.shape[:2])
                f.write(f"0 {' '.join(map(str, normalized_bbox))}\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Convert ScanTailor data to YOLO format.")
    ...
    parser.add_argument("--output", type=str, default="datasets/yolo2", help="Path to the output directory for YOLO formatted images.")
    parser.add_argument("--metadata_folder", type=str, default="datasets/scantailor_data", help="Path to the folder containing metadata JSON files.")
    ...
    for project in train_projects:
        ...
        process_project_single_page(input, output, project, metadata_folder, split="train")
        process_project_double_page(input, output, project, metadata_folder, split="train")
    ...
    for project in val_projects:
        ...
        process_project_single_page(input, output, project, metadata_folder, split="val")

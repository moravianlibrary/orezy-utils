...
def walk_through_dataset():
    ...
    books = [d for d in os.listdir("/Users/lucienovotna/Documents/ai-orezy-compressed") if os.path.isdir(os.path.join("/Users/lucienovotna/Documents/ai-orezy-compressed", d))]

    for book in books:
        with open(os.path.join("/Users/lucienovotna/Documents/ai-orezy-compressed", book, "scanTailor", "metadata.json"), "r") as f:
            data = f.read()
        ...
        for split in ["train", "val", "test"]:
            ...
            for file in files:
                if file.startswith(book + "-"):
                    ...
                    break
        for image in images:
            label_file = os.path.join(base_path, "labels", directory, image.replace(".jpg", ".txt"))

...
def get_feature_vec_img_properties(image):
    ...
    h, w = image.shape[:2]
    scale = 800 / max(h, w)
    if scale < 1: image = cv2.resize(image, (int(w*scale), int(h*scale)))
...
def visualize(clusters, title):
    ...
    largest_cluster_size = min(largest_cluster_size, 20)  # Ensure maximally 20 images per row
    fig, axs = plt.subplots(len(clusters), largest_cluster_size, figsize=(10, len(clusters) * 2))

    for ax_row, (label, images) in zip(axs, clusters.items()):
        for i, img_path in enumerate(images):
            if i == 0:
                ax_row[i].set_title(f"Cluster {label} ({len(images)} images)", fontsize=10)
...
if __name__ == "__main__":
    ...
    feature_labels = cluster_features(files, get_feature_vec_img_properties, n_clusters=3)
